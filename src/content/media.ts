...
> = {
   ...
   *
   * ⚠️ THERE IS NOW A REAL BRAND MARK, and this slot is deliberately NOT
   * pointing at it yet: `public/brand/pearl-mark.svg` (an open scallop holding a
   * pearl, added in v0.8.0). It already drives the favicon
   * (`src/app/icon.png`, generated by `scripts/brand/generate-icon.mjs`) and the
   * OG share card.
   *
   * Swapping it in here is not a one-line change, which is why it was not done
   * in the same pass: `BrandMark` renders this slot through `next/image` with
   * `rounded-full object-cover ring-1 ring-brand/30` — a circular crop with a
   * ring, which is right for a photograph and wrong for a line mark (it would
   * clip the dish and draw a ring around thin air). Doing it properly means
   * teaching `brand-mark.tsx` a second, un-cropped branch for an inline SVG.
   * That is a component change, not a content change. Until then the header
   * keeps her face, which is honestly not a bad look for a personal practice.
   */
  logo: {
    src: "/images/pnina-portrait.jpg",
    alt: "פנינה פאף",
    note: "כרגע תמונת הפרופיל. יש כבר סימן מותג (צדפה ופנינה) ב-public/brand/pearl-mark.svg — ההחלפה דורשת שינוי ב-brand-mark.tsx.",
    width: 320,
    height: 320,
  },
  /**
